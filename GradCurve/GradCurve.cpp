...
int32_t *rgblab; //LUT Lab
int32_t *labrgb; //LUT Lab

struct GradCurveData {
	VSNodeRef * node;
	const VSVideoInfo * vi;
	// xekon add vdub
	int32_t rvalue[3][256];
	int gvalue[3][256];
	int bvalue[256];
	int ovalue[5][256];
	int channel_mode;
...
void PreCalcLut()
{
	...
	rgblab = new int32_t[16777216];
	labrgb = new int32_t[16777216];

	count = 0;
	for (r = 0; r<256; r++) {
		for (g = 0; g<256; g++) {
			for (b = 0; b<256; b++) {
				if (r > 10) { rr = int32_t(pow(((r << 4) + 224.4), (2.4))); }
				else { rr = int32_t((r << 4)*9987.749); }
				if (g > 10) { gg = int32_t(pow(((g << 4) + 224.4), (2.4))); }
				else { gg = int32_t((g << 4)*9987.749); }
				if (b > 10) { bb = int32_t(pow(((b << 4) + 224.4), (2.4))); }
				else { bb = int32_t((b << 4)*9987.749); }
				x = int32_t((rr + 6.38287545) / 12.7657509 + (gg + 7.36187255) / 14.7237451 + (bb + 14.58712555) / 29.1742511);
				y = int32_t((rr + 12.37891725) / 24.7578345 + (gg + 3.68093628) / 7.36187256 + (bb + 36.4678139) / 72.9356278);
				z = int32_t((rr + 136.1678335) / 272.335667 + (gg + 22.0856177) / 44.1712354 + (bb + 2.76970661) / 5.53941322);
				//XYZ to Lab
				if (x>841776) { rr = int32_t(pow((x), (0.33333333333333333333333333333333))*21.9122842); }
				else { rr = int32_t((x + 610.28989295) / 1220.5797859 + 1379.3103448275862068965517241379); }
				if (y>885644) { gg = int32_t(pow((y), (0.33333333333333333333333333333333))*21.5443498); }
				else { gg = int32_t((y + 642.0927467) / 1284.1854934 + 1379.3103448275862068965517241379); }
				if (z>964440) { bb = int32_t(pow((z), (0.33333333333333333333333333333333))*20.9408726); }
				else { bb = int32_t((z + 699.1298454) / 1398.2596908 + 1379.3103448275862068965517241379); }
				x = int32_t(((gg + 16.90331) / 33.806620) - 40.8);
				y = int32_t(((rr - gg + 7.23208898) / 14.46417796) + 119.167434);
				z = int32_t(((gg - bb + 19.837527645) / 39.67505529) + 135.936123);
	...
	for (x = 0; x<256; x++) {
		for (y = 0; y<256; y++) {
			for (z = 0; z<256; z++) {
				gg = x * 50 + 2040;
				rr = int32_t(y*21.392519204 - 2549.29163142 + gg);
				bb = int32_t(gg - z*58.67940678 + 7976.6510628);
				if (gg>3060) { g1 = int32_t(gg*gg / 32352.25239*gg); }
				else { g1 = int32_t(x*43413.9788); }
				if (rr>3060) { r1 = int32_t(rr*rr / 34038.16258*rr); }
				else { r1 = int32_t(rr*825.27369 - 1683558); }
				if (bb>3060) { b1 = int32_t(bb*bb / 29712.85911*bb); }
				else { b1 = int32_t(bb*945.40885 - 1928634); }
				//XYZ to RGB
				rr = int32_t(r1*16.20355 + g1*-7.6863 + b1*-2.492855);
				gg = int32_t(r1*-4.84629 + g1*9.37995 + b1*0.2077785);
				bb = int32_t(r1*0.278176 + g1*-1.01998 + b1*5.28535);
...
void ImportCurve(GradCurveData * d) // import curves
{
	...
	int stor[1280];
	int temp[1280];
	int32_t lSize = 0;//xekon set 0 default, clear uninitialized error
	int beg;
	int cv;
